...
MAX_SEND = intval("MAX_DEALS_PER_CHECK", 10)
...
async def storage_init():
    if DATABASE_URL and asyncpg:
        try:
            ...
            try:
                await c.execute("""CREATE TABLE IF NOT EXISTS sent_deals(
            ...
            return
        except Exception as e: log.warning("⚠️ PostgreSQL недоступен: %s", e)
    os.makedirs(CACHE_DIR, exist_ok=True)
    try:
        if os.path.exists(CACHE_FILE):
            with open(CACHE_FILE, encoding="utf-8") as f: data=json.load(f)
            globals()["sent_cache"]={str(x) for x in data} if isinstance(data,list) else set()
        log.info("💾 Временный cache: %s | %s товаров", CACHE_FILE, len(sent_cache))
    except Exception as e: log.warning("⚠️ Cache load: %s", e)

def save_cache():
    try:
        os.makedirs(CACHE_DIR, exist_ok=True)
        tmp=CACHE_FILE+".tmp"
        with open(tmp,"w",encoding="utf-8") as f: json.dump(list(sent_cache)[-MAX_CACHE:],f,ensure_ascii=False)
        os.replace(tmp,CACHE_FILE)
    except Exception as e: log.warning("⚠️ Cache не сохранён: %s",e)
...
async def is_sent(pid):
    ...
    if DATABASE_URL and asyncpg:
        try:
            c=await asyncpg.connect(DATABASE_URL)
            try: return await c.fetchval("SELECT 1 FROM sent_deals WHERE product_id=$1",pid) is not None
            finally: await c.close()
        except Exception as e: log.warning("DB is_sent: %s",e)
    return pid in sent_cache

async def mark_sent(d):
    pid=str(d["id"]); sent_cache.add(pid)
    if len(sent_cache)>MAX_CACHE: globals()["sent_cache"]=set(list(sent_cache)[-MAX_CACHE:])
    if DATABASE_URL and asyncpg:
        try:
            c=await asyncpg.connect(DATABASE_URL)
            try:
                await c.execute("""INSERT INTO sent_deals(product_id,title,discount,price_usd,link)
                VALUES($1,$2,$3,$4,$5) ON CONFLICT(product_id) DO NOTHING""",pid,d["title"],d["discount_percent"],d["discount_price_usd"],d["link"])
            finally: await c.close()
            return
        except Exception as e: log.warning("DB mark_sent: %s",e)
    save_cache()
...
async def stored_count():
    if DATABASE_URL and asyncpg:
        try:
            c=await asyncpg.connect(DATABASE_URL)
            try: return int(await c.fetchval("SELECT COUNT(*) FROM sent_deals") or 0)
            finally: await c.close()
        except: pass
...
def parse_card(card,i):
    try:
        ...
        return {"id":pid(u),"title":t,"brand":"iHerb","orig_price_usd":old,"discount_price_usd":cur,"discount_percent":int(d),"link":u}
...
async def fetch_deals():
    ...
    deals.sort(key=lambda x:(x["discount_percent"],-x["discount_price_usd"]),reverse=True)
...
def message(d):
    ...
    cost=round(cur*KZT); sale=round(cost*(1+MARKUP/100)); profit=sale-cost
